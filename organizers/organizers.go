...
func MakeOrganizerIndexFile(city string, year string) (err error) {
	...
	templateBox, err := rice.FindBox("organizer-templates")
	if err != nil {
		return errors.Wrap(err, "content template find failed")
	}
	// get file contents as string
	templateString, err := templateBox.String("index.md.tmpl")
	if err != nil {
		return errors.Wrap(err, "Cannot load organizer index template")
	}

	t, err := template.New("index.md").Parse(templateString)
	if err != nil {
		return errors.Wrap(err, "Cannot load organizer template")
	}

	f, err := os.Create(filepath.Join((paths.EventNewContentPath(city, year)), "organizers", "index.md"))
	if err != nil {
